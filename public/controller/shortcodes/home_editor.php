...
class Mp_cf_home_editor_public
{
	...
	public function mp_cf_review_submissions_shortcode($data){
		$review_submissions = get_posts(array(
			'post_type' => 'cf-submitted-content',
			'post_status' => 'pending',
			'posts_per_page' => -1
		));
		
		if(isset($data['display']) && $data['display'] === 'count') echo count($review_submissions);
		
		else {
			wp_enqueue_style( 'mp-factory-request-content-style', mp_cf_PLAGIN_URL . 'public/css/mp-factory-request-content.css', false, '1.0', 'all' ); 

			wp_enqueue_style( 'mp-factory-requested-articles-style', mp_cf_PLAGIN_URL . 'public/css/mp-factory-requested-articles.css', false, '1.0', 'all' ); 
			include_once mp_cf_PLAGIN_DIR . 'public/partials/editor/review_submissions/index.php';
		}
	}
